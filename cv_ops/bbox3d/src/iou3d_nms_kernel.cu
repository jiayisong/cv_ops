...
__device__ inline float dist_3d(const float *box_a, const float *box_b){
    // params box_a: [x, y, z, dx, dy, dz, heading]
    // params box_b: [x, y, z, dx, dy, dz, heading]
    return (box_a[0] - box_b[0]) * (box_a[0] - box_b[0]) + (box_a[2] - box_b[2]) * (box_a[2] - box_b[2]);
}
...
__global__ void boxes_iou_3d_kernel(const int batch_size, const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_iou){
    // params boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params boxes_b: (M, 7) [x, y, z, dx, dy, dz, heading]
    const int N = batch_size*num_a*num_b;
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < N; i += blockDim.x * gridDim.x)
  {
    const int batch_idx = i / (num_a * num_b);
    const int a_idx = (i % (num_a * num_b)) / num_b;
    const int b_idx = (i % (num_a * num_b)) % num_b;
    const float * cur_box_a = boxes_a + batch_idx * num_a * 7 + a_idx * 7;
    const float * cur_box_b = boxes_b + batch_idx * num_b * 7 + b_idx * 7;
    float cur_iou_3d = iou_3d(cur_box_a, cur_box_b);
    ans_iou[batch_idx * num_a * num_b + a_idx * num_b + b_idx] = cur_iou_3d;
   }
}
...
__global__ void nms_dist_kernel(const int boxes_num, const float nms_overlap_thresh,
                           const float *boxes, unsigned long long *mask){
    //params: boxes (N, 7) [x, y, z, dx, dy, dz, heading]
    //params: mask (N, N/THREADS_PER_BLOCK_NMS)
    ...
    if (threadIdx.x < row_size) {
        ...
        for (i = start; i < col_size; i++) {
            if (dist_3d(cur_box, block_boxes + i * 7) < (nms_overlap_thresh * nms_overlap_thresh)){
                t |= 1ULL << i;
            }
        }
        const int col_blocks = DIVUP(boxes_num, THREADS_PER_BLOCK_NMS);
...
void boxesiou3dLauncher(const int batch_size, const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_iou){
    ...
    //dim3 threads(THREADS_PER_BLOCK, THREADS_PER_BLOCK);
  const int N = batch_size*num_a*num_b;
  int optimal_block_num = (N + 511) / 512;
  int max_block_num = 4096;
    boxes_iou_3d_kernel<<<min(optimal_block_num, max_block_num), 512>>>(batch_size, num_a, boxes_a, num_b, boxes_b, ans_iou);
#ifdef DEBUG
    //cudaDeviceSynchronize();  // for using printf in kernel function
#endif
}
...
void nmsdistLauncher(const float *boxes, unsigned long long * mask, int boxes_num, float nms_overlap_thresh){
    dim3 blocks(DIVUP(boxes_num, THREADS_PER_BLOCK_NMS),
                DIVUP(boxes_num, THREADS_PER_BLOCK_NMS));
    dim3 threads(THREADS_PER_BLOCK_NMS);
    nms_dist_kernel<<<blocks, threads>>>(boxes_num, nms_overlap_thresh, boxes, mask);
}